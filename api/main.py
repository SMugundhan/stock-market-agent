...
def get_graph():
    ...
    if graph is None:
        raise HTTPException(
            status_code=503,
            detail="Analysis Pipeline not ready -- server starting up"
        )
    return graph
...
async def analyze_stock(request: AnalysisRequest, graph = Depends ( get_graph ) ):
    """
    ...
    # graph is now injected through fn calls to avoid the testing failures refer day 16
    # graph = app_state . get ( "graph" )
